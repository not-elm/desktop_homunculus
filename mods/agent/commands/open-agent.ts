...
try {
  ...
  try {
    ...
    // If RPC fails (service not ready), default to expanded mode
  }

  await Webview.open({
    source: webviewSource.local("agent:session-ui"),
    size: isSession ? [0.6, 0.8] : [0.85, 0.8],
    viewportSize: isSession ? [400, 500] : [640, 500],
    offset: isSession ? [0.8, -0.5] : [1.2, -0.3],
