...
import type { TransformArgs, Vec2 } from './math';
 ...
 * ```
 */
export type WebviewSource = WebviewSourceUrl | WebviewSourceHtml | WebviewSourceLocal;
...
export function isWebviewSourceInfoLocal(
  source: WebviewSourceInfo,
): source is WebviewSourceInfoLocal {
  return source.type === 'local';
}
...
export function isWebviewSourceInfoHtml(
  source: WebviewSourceInfoHtml,
): source is WebviewSourceInfoHtml {
  return source.type === 'html';
}
...
export class Webview {
  ...
  async linkedPersona(): Promise<Persona | undefined> {
    const response = await host.get(host.createUrl(`webviews/${this.entity}/linked-persona`));
  ...
  async setLinkedPersona(personaId: string): Promise<void> {
    await host.put(host.createUrl(`webviews/${this.entity}/linked-persona`), { personaId });
  }
  ...
  async unlinkPersona(): Promise<void> {
    await host.deleteMethod(host.createUrl(`webviews/${this.entity}/linked-persona`));
  }
