...
import type { Transform, Vec2 } from './math';
...
export namespace entities {
   ...
   * ```
   */
  export async function setTransform(entity: number, transform: Partial<Transform>): Promise<void> {
   ...
   * ```
   */
  export async function findByName(name: string, options?: FindOptions): Promise<number> {
    const response = await host.get(
      host.createUrl('entities', {
        name,
        ...options,
      }),
    );
   ...
   */
  export type MoveTarget = MoveTargetWorld | MoveTargetViewport;
   ...
   * ```
   */
  export async function tweenPosition(
    entityId: number,
    request: TweenPositionRequest,
  ): Promise<void> {
   ...
   * ```
   */
  export async function tweenRotation(
    entityId: number,
    request: TweenRotationRequest,
  ): Promise<void> {
