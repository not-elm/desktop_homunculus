...
export namespace stt {
   ...
   * ```
   */
  export function isSttError(e: unknown, code?: SttErrorCode): e is HomunculusApiError {
  ...
  export async function recognize(
    ...
    const response = await host.post(host.createUrl('stt/recognize'), options ?? {}, signal);
  ...
  export namespace models {
    ...
    export async function cancelDownload(modelSize: SttModelSize): Promise<boolean> {
      try {
        await host.deleteMethod(host.createUrl('stt/models/download', { modelSize }));
        return true;
      } catch (e) {
  ...
  export class DownloadStream
    implements AsyncIterable<DownloadEvent>, PromiseLike<ModelDownloadResponse>
  {
    ...
    then<TResult1 = ModelDownloadResponse, TResult2 = never>(
      ...
      const promise = (async (): Promise<ModelDownloadResponse> => {
        const response = await host.post(host.createUrl('stt/models/download'), {
          modelSize: this.options.modelSize,
        });
  ...
  export namespace ptt {
     ...
     * ```
     */
    export async function start(options?: StartOptions): Promise<PttSession> {
      const response = await host.post(host.createUrl('stt/ptt/start'), options ?? {});
      ...
      return {
        ...
        modelSize,
        stop: async () => {
          const stopResponse = await host.post(host.createUrl(`stt/ptt/${sessionId}/stop`), {});
