...
import * as hmcsClient from '../hmcs-client.js';
...
describe('seedFromHmcs', () => {
  test('inserts personas; for those with matching agents, calls writePersonaFiles', async () => {
    ...
    vi.spyOn(hmcsClient, 'getPersonas').mockResolvedValue(personas as any);
  ...
  test('warns once when persona has no matching agent', async () => {
    vi.spyOn(hmcsClient, 'getPersonas').mockResolvedValue([
      { id: 'bob', name: 'Bob', metadata: {}, spawned: true },
    ] as any);
    const deps = makeDeps();
    await seedFromHmcs(deps as any);
  ...
  test('does not throw when HMCS getPersonas fails (logs warning)', async () => {
    vi.spyOn(hmcsClient, 'getPersonas').mockRejectedValue(new Error('ECONNREFUSED'));
    const deps = makeDeps();
