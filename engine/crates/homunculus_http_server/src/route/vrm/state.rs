...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "VRM state", body = Object),
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "VRM state updated"),
        (status = 404, description = "Entity not found"),
    ),
)]
pub async fn put(
...
mod tests {
    ...
    async fn test_get_vrm_state() {
        ...
        let entity = app.world_mut().spawn(CharacterState::default()).id();
        app.update();
        ...
        let request = axum::http::Request::put(format!("/vrm/{}/state", entity.to_bits()))
            ...
            .body(axum::body::Body::from(
                serde_json::to_string(&PutBody {
                    state: CharacterState("dancing".to_string()),
                })
                .unwrap(),
            ))
            .unwrap();
