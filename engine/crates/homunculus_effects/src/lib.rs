...
impl Plugin for HomunculusEffectsPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(StampEffectsPlugin)
...
fn initialize_camera_position(
    ...
    cameras: Query<
        (Entity, &Camera, &RenderTarget, &Transform, &GlobalTransform),
    ...
    WINIT_WINDOWS.with(|winit_windows| {
        ...
        cameras
            .iter()
            .for_each(|(entity, camera, target, camera_tf, gtf)| {
                let RenderTarget::Window(WindowRef::Entity(window_entity)) = target else {
                    return;
                };
                ...
                let Some(center) = camera.logical_viewport_size() else {
                    return;
                };
                let center = center / 2.;
