...
impl MascotTracker<'_, '_> {
    ...
    /// Computes the sitting transform using a provided hips Y offset instead of the live bone.
    ///
    /// This is used by the sitting system's `HipsAnchor` to decouple position
    /// tracking from animation state transitions. Only the Y offset is compensated
    /// (scaled by `adjust`); the X position is determined solely by the viewport
    /// sitting position.
    pub fn tracking_with_fixed_offset(
        &self,
        vrm: Entity,
        pos: GlobalViewport,
        hips_offset_y: f32,
        ...
        let mut cursor_pos =
            self.camera
                .to_world_by_viewport(window_entity, viewport_pos, tf.translation)?;
        let mut new_tf = *tf;
        cursor_pos.y -= hips_offset_y * adjust;
        new_tf.translation = cursor_pos;
        Some(new_tf)
    }
}
