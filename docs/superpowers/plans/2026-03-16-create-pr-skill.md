...
**Goal:** Create a Claude Code skill that automates PR creation using the project's PR template with diff-based plausibility validation.

**Architecture:** Single SKILL.md file at `.claude/skills/create-pr/SKILL.md`. 5-step linear flow: Preflight → Problem input → Plausibility check → Draft generation/approval → Push and create/edit PR.

**Tech Stack:** Claude Code skill (Markdown), git CLI, gh CLI

**Spec:** `docs/superpowers/specs/2026-03-16-create-pr-skill-design.md`

---

## Chunk 1: Implement SKILL.md
...
- `.github/pull_request_template.md` — the PR template this skill uses
- `.claude/skills/review-docs/SKILL.md` — structural reference for skill format
- `docs/superpowers/specs/2026-03-16-create-pr-skill-design.md` — the design spec

- [ ] **Step 1: Read reference files**

Read all three reference files listed above to understand the PR template structure, existing skill conventions, and the full design spec.

- [ ] **Step 2: Create `.claude/skills/create-pr/SKILL.md`**
...
Read back `.claude/skills/create-pr/SKILL.md` and verify:
- Frontmatter has correct `name` and `description`
- All 5 steps are present with correct content
- Preflight has all 8 checks including base branch fallback
- Step 3 includes large diff handling
- Step 4 reads the template at runtime
- Step 5 includes `--base` flag and push error handling
- Checklist notation uses `[x]` / `[ ]` (not zero-width characters)
...
git commit -m "feat: add create-pr skill for automated PR creation"
```
