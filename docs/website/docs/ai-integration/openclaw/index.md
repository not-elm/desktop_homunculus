...
`@hmcs/openclaw-plugin` bridges [OpenClaw](https://docs.openclaw.ai) agents with Desktop Homunculus characters. Install it into your OpenClaw setup and your DH personas become OpenClaw workspaces; agent replies render on the matching character — speech bubbles, and spoken aloud if the VoiceVox MOD is installed.
...
- **[`@hmcs/persona` MOD](/mods/persona)** with at least one persona created. The plugin syncs DH personas to OpenClaw workspaces; it has nothing to do until a persona exists.
...
The plugin does not create OpenClaw agents automatically. For every DH persona you want to drive, register a matching agent whose ID equals the persona ID:

```bash
openclaw agents add <persona-id>
```

You can list your DH personas from the persona MOD UI or via the HTTP API:
...
The plugin reads configuration from OpenClaw's plugin config. Defaults work for the standard DH setup:
...
5. Send a message. The matching DH character should react — the reply appears as a speech bubble (and is spoken if VoiceVox is installed).
