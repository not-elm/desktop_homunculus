...
- A **startup script** (`main` field) -- runs automatically as a Node.js child process when the app starts
...
- **Run a startup script** — Execute a long-running TypeScript process when the app launches (declared via the `main` field). Startup scripts typically set up characters and event listeners. The `@hmcs/menu` MOD uses a startup script to initialize the right-click menu overlay.
