...
- `@hmcs/persona` MOD はこの MOD に依存しており、ペルソナのアニメーションに `vrma:idle-maid`、`vrma:grabbed`、`vrma:idle-sitting` を使用しています。
- この MOD を削除すると、ペルソナ MOD およびこれらのアセット ID を参照する他の MOD が動作しなくなります。
