...
```shell
# 1. すべてのツールをインストール（Rust、Node、CEF フレームワーク）
make setup

# 2. デバッグモードでアプリを起動（ホットリロードと DevTools 付き）
make debug
```

利用可能なすべてのコマンドについては、ルートの `Makefile` をご覧ください。
...
MOD の作成方法については [MOD 開発ガイド](/docs/mod-development/quick-start)をご覧ください。

### ユーザースキル

Desktop Homunculus キャラクターとのエンドユーザー体験を向上させる Claude Code スキルのコントリビューションを歓迎しています。スキルは MCP ツール呼び出しをワークフローに連鎖させます。例えば、スキルによってキャラクターにスライドとナレーション付きのレクチャーを行わせることができます。

利用可能なスキル、インストール方法、独自のスキルの作成方法については、[スキルカタログとコントリビューションガイド](https://github.com/not-elm/desktop-homunculus/tree/main/skills)をご覧ください。
...
現在の使用方法と API の例については [コンポーネントライブラリガイド](/docs/mod-development/webview-ui/component-library)をご覧ください。
...
[GitHub Discussion](https://github.com/not-elm/desktop-homunculus/discussions) を開いてください。喜んでお手伝いします。
