...
Bin コマンドは、MOD が `package.json` の `bin` フィールドを通じて公開するオンデマンドスクリプトです。起動時に自動実行されるサービスとは異なり、bin コマンドは HTTP API 経由で明示的に呼び出された場合にのみ実行されます。

`package.json` での `bin` フィールドの宣言方法については[パッケージ設定](./project-setup/package-json.md#bin-commands)を参照してください。

## Bin コマンドスクリプトの作成

### シバン行

すべての TypeScript bin コマンドは、コンパイルステップなしで直接実行を可能にするシバン行で始める必要があります：
...
シバンはシステムに対し、`--experimental-strip-types` フラグを使って Node.js でファイルを実行するよう指示します。このフラグはランタイム時に TypeScript 構文を除去します。`/// <reference types="node" />` ディレクティブは Node.js の型定義（`process.stdin` など）を提供します。

:::warning
`--experimental-strip-types` には Node.js 22 以降が必要です。セットアップ手順は[インストール](/docs/getting-started/installation)を参照してください。
:::

### `input.parse` による入力の解析

Bin コマンドは stdin 経由で JSON として入力を受け取ります。SDK は `@hmcs/sdk/commands` から `input.parse` を提供し、Zod スキーマを使って入力の読み取り、解析、バリデーションを行います。
...
Bin コマンドは stdout と stderr を通じて結果を伝達します：
...
`@hmcs/sdk` は他の MOD スクリプトから bin コマンドを呼び出すための 2 つの便利な関数を提供しています：
...
詳細は [Mods API](./sdk/mods-api) リファレンスを参照してください。

## 完全な例

入力パラメータに基づいて挨拶メッセージを構築する完全な bin コマンドです。
