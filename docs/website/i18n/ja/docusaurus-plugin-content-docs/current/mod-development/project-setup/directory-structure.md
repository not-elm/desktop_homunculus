...
`package.json` の `"bin"` フィールドで公開される MOD コマンドです。HTTP API（`POST /commands/execute`）を通じて明示的に呼び出された場合にのみ実行されます。慣例的なディレクトリ名は `commands/` または `bin/` です。
...
- **MOD コマンドスクリプトは `commands/` または `bin/` に配置します。** どちらも公式 MOD で一般的です。
