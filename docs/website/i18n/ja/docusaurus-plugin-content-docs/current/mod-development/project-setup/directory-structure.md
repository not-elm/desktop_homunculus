...
設定パネルやその他の UI を提供する MOD は、React アプリと MOD コマンドを追加します：

```text
my-settings-mod/
├── package.json           # アセット、メニュー、MOD コマンドを宣言
├── commands/
│   └── open-ui.ts         # MOD コマンドスクリプト（HTTP API 経由で呼び出し）
└── ui/
...
`package.json` の `"bin"` フィールドで公開される MOD コマンドです。HTTP API（`POST /commands/execute`）を通じて明示的に呼び出された場合にのみ実行されます。慣例的なディレクトリ名は `commands/` です。
...
:::info
この例にはルートに `index.ts` がありません。MOD コマンドと UI パネルのみを使用しています。MOD はサービス、UI アプリ、MOD コマンドの両方のパターンを同一パッケージ内で組み合わせることもできます。
:::
...
- **MOD コマンドスクリプトは `commands/` に配置します。** 公式 MOD 全体での標準です。
